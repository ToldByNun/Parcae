...
# Requires: iscc (Inno Setup 6) on PATH, or ISCC env pointing to ISCC.exe
set -euo pipefail
...
# Normalize paths for ISCC on Windows (Git Bash).
stage_win="${stage}"
out_win="${out}"
...
echo "Built Windows installer flavor=${flavor} -> ${out}"
