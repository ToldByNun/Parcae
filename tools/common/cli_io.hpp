...
public:
    ...
    static constexpr int kExitFail = 1;
    static constexpr int kExitUsage = 2;

    [[nodiscard]] static StatusOr<std::string> read_all_utf8(std::string_view path_or_dash) {
        if (path_or_dash == "-") {
    ...
    [[nodiscard]] static StatusOr<std::string> read_file_utf8(const std::filesystem::path& path) {
...
#pragma warning(pop)
        ...
        return Context{std::filesystem::weakly_canonical(root)};
    }

    [[nodiscard]] static StatusOr<std::vector<std::size_t>> parse_size_list(std::string_view text) {
    ...
    [[nodiscard]] static StatusOr<std::vector<int>> parse_int_list(std::string_view text) {
    ...
    [[nodiscard]] static std::vector<std::string> argv_tail(int argc, char** argv) {
