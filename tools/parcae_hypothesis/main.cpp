...
[[nodiscard]] int fail_status(
    ...
    const int plain =
        (code == ToolErrorCode::Policy) ? CliIo::kExitUsage : plain_exit_fallback;
...
[[nodiscard]] int cmd_init(
    ...
    if (!workspace.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, workspace.status().message(), CliIo::kExitUsage);
    }
    StatusOr<std::string> id = CliIo::require_option(args, "--id");
    if (!id.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, id.status().message(), CliIo::kExitUsage);
    }
    ...
    if (!existing.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, existing.status().message(), CliIo::kExitUsage);
    }
    ...
    if (!method_json.empty()) {
        ...
        if (!parsed.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, parsed.status().message(), CliIo::kExitUsage);
        }
    ...
    StatusOr<HypothesisRecord> draft = HypothesisRecord::make_draft(
        ...
        id.value(),
        utc,
        CliIo::optional_option(args, "--title"),
        std::move(method));
    if (!draft.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Schema, draft.status().message(), CliIo::kExitUsage);
    }
...
[[nodiscard]] int cmd_propose(
    ...
    if (!method.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, method.status().message(), CliIo::kExitUsage);
    }
    ...
    StatusOr<std::filesystem::path> path =
        WorkspacePaths::hypothesis_file(ctx.data_root(), workspace.value(), id.value());
    if (!path.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, path.status().message(), CliIo::kExitUsage);
    }
    ...
    if (std::filesystem::exists(path.value())) {
        ...
        if (!loaded.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, loaded.status().message(), CliIo::kExitFail);
        }
        record = std::move(loaded.value());
    } else {
        StatusOr<HypothesisRecord> draft = HypothesisRecord::make_draft(
            workspace.value(), id.value(), utc, CliIo::optional_option(args, "--title"), method.value());
        if (!draft.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, draft.status().message(), CliIo::kExitUsage);
        }
    ...
    if (!source_json.empty()) {
        ...
        if (!source.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, source.status().message(), CliIo::kExitUsage);
        }
        Status source_ok = record.set_source(std::move(source.value()));
        if (!source_ok.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, source_ok.message(), CliIo::kExitUsage);
        }
    }
...
[[nodiscard]] int cmd_show(
    ...
    StatusOr<HypothesisRecord> record =
        HypothesisRecord::load(ctx.data_root(), workspace.value(), id.value());
    if (!record.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, record.status().message(), CliIo::kExitFail);
    }
...
[[nodiscard]] int cmd_list(
    ...
    for (const std::string& id : ids.value()) {
        ...
        if (!record.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, record.status().message(), CliIo::kExitFail);
        }
        rows.push_back({
            {"id", record.value().id()},
...
[[nodiscard]] int cmd_score(
    ...
    if (!input_path.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, input_path.status().message(), CliIo::kExitUsage);
    }
    ...
    StatusOr<HypothesisRecord> loaded =
        HypothesisRecord::load(ctx.data_root(), workspace.value(), id.value());
    if (!loaded.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, loaded.status().message(), CliIo::kExitFail);
    }
    ...
    if (!ScoreId::from_string(score_id).ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, "Unknown score_id: " + score_id, CliIo::kExitUsage);
    }

    StatusOr<std::string> source = CliIo::read_all_utf8(input_path.value());
    if (!source.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, source.status().message(), CliIo::kExitUsage);
    }
    StatusOr<std::vector<Index29>> cipher =
        load_input_indices(ctx, source.value(), input_mode);
    if (!cipher.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, cipher.status().message(), CliIo::kExitFail);
    }

    StatusOr<TransformEnvelope> envelope =
        TransformEnvelope::from_json(record.method());
    if (!envelope.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Schema, envelope.status().message(), CliIo::kExitFail);
    }
    StatusOr<std::vector<Index29>> plain =
        ToolApi::apply_to_indices(cipher.value(), envelope.value());
    if (!plain.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, plain.status().message(), CliIo::kExitFail);
    }

    StatusOr<double> value = ToolApi::score(ctx, plain.value(), score_id);
    if (!value.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, value.status().message(), CliIo::kExitFail);
    }

    const std::string utc = resolve_utc(args);
    StatusOr<std::string> latin = ToolApi::to_latin(ctx, plain.value());
    if (!latin.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, latin.status().message(), CliIo::kExitFail);
    }
    ...
    if (record.status() == HypothesisStatus::Draft) {
        ...
        if (!to_proposed.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, to_proposed.message(), CliIo::kExitFail);
        }
    }
    if (record.status() == HypothesisStatus::Proposed) {
        Status to_scored = record.set_status(HypothesisStatus::Scored);
        if (!to_scored.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, to_scored.message(), CliIo::kExitFail);
        }
    }
...
[[nodiscard]] int cmd_set_status(
    ...
    if (!status_text.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, status_text.status().message(), CliIo::kExitUsage);
    }
    ...
    if (!next.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, next.status().message(), CliIo::kExitUsage);
    }
...
int main(int argc, char** argv) {
    ...
    if (!ctx.ok()) {
        return fail("hypothesis", json_mode, ToolErrorCode::Io, ctx.status().message(), CliIo::kExitUsage);
    }
