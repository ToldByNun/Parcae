...
[[nodiscard]] std::string resolve_utc(const std::vector<std::string>& args) {
    const std::string flag = parcae::cli::optional_option(args, "--utc");
    if (!flag.empty()) {
        return flag;
    }
...
[[nodiscard]] StatusOr<nlohmann::json> parse_json_object(
...
[[nodiscard]] StatusOr<nlohmann::json> load_method_json(const std::vector<std::string>& args) {
    ...
    const std::string inline_json = optional_option(args, "--method-json");
    const std::string file_path = optional_option(args, "--method-file");
    ...
    if (!file_path.empty()) {
        StatusOr<std::string> text = read_all_utf8(file_path);
        if (!text.ok()) {
            return text.status();
        }
...
[[nodiscard]] StatusOr<std::vector<Index29>> load_input_indices(
...
[[nodiscard]] int cmd_init(
    ...
    StatusOr<std::string> workspace = require_option(args, "--workspace");
    if (!workspace.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, workspace.status().message(), kExitUsage);
    }
    ...
    StatusOr<WorkspaceManifest> ws = WorkspaceManifest::ensure(ctx.data_root(), workspace.value(), utc);
    if (!ws.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, ws.status().message(), kExitUsage);
    }

    StatusOr<std::filesystem::path> existing =
        WorkspacePaths::hypothesis_file(ctx.data_root(), workspace.value(), id.value());
    if (!existing.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, existing.status().message(), kExitUsage);
    }
    if (std::filesystem::exists(existing.value())) {
        return fail(
            ...
            json_mode,
            ToolErrorCode::Usage,
            "Hypothesis already exists: " + id.value(),
            kExitUsage);
    }
    ...
    const std::string method_json = optional_option(args, "--method-json");
    if (!method_json.empty()) {
        StatusOr<nlohmann::json> parsed = parse_json_object(method_json, "--method-json");
        if (!parsed.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, parsed.status().message(), kExitUsage);
        }
    ...
    StatusOr<HypothesisRecord> draft = HypothesisRecord::make_draft(
        ...
        id.value(),
        utc,
        optional_option(args, "--title"),
        std::move(method));
    if (!draft.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Schema, draft.status().message(), kExitUsage);
    }
    draft.value().recompute_method_digest();
    Status stored = draft.value().store(ctx.data_root());
    if (!stored.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, stored.message(), kExitFail);
    }
...
[[nodiscard]] int cmd_propose(
    ...
    StatusOr<std::filesystem::path> path =
        WorkspacePaths::hypothesis_file(ctx.data_root(), workspace.value(), id.value());
    if (!path.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, path.status().message(), kExitUsage);
    }
    ...
    if (std::filesystem::exists(path.value())) {
        ...
        if (!loaded.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, loaded.status().message(), kExitFail);
        }
        record = std::move(loaded.value());
    } else {
        StatusOr<HypothesisRecord> draft = HypothesisRecord::make_draft(
            workspace.value(), id.value(), utc, optional_option(args, "--title"), method.value());
        if (!draft.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, draft.status().message(), kExitUsage);
        }
    ...
    if (!method_ok.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Schema, method_ok.message(), kExitUsage);
    }
    const std::string title = optional_option(args, "--title");
    if (!title.empty()) {
        record.set_title(title);
    }
    const std::string rationale = optional_option(args, "--rationale");
    if (!rationale.empty()) {
        record.set_rationale(rationale);
    }
    const std::string source_json = optional_option(args, "--source-json");
    ...
    if (record.status() == HypothesisStatus::Draft) {
        ...
        if (!tr.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, tr.message(), kExitFail);
        }
    }
...
[[nodiscard]] int cmd_show(
    ...
    StatusOr<HypothesisRecord> record =
        HypothesisRecord::load(ctx.data_root(), workspace.value(), id.value());
    if (!record.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, record.status().message(), kExitFail);
    }
...
[[nodiscard]] int cmd_list(
    ...
    if (!ids.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, ids.status().message(), kExitFail);
    }
    ...
    for (const std::string& id : ids.value()) {
        ...
        if (!record.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, record.status().message(), kExitFail);
        }
        rows.push_back({
            {"id", record.value().id()},
...
[[nodiscard]] int cmd_score(
    ...
    StatusOr<HypothesisRecord> loaded =
        HypothesisRecord::load(ctx.data_root(), workspace.value(), id.value());
    if (!loaded.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, loaded.status().message(), kExitFail);
    }
    HypothesisRecord record = std::move(loaded.value());

    std::string score_id = optional_option(args, "--score-id");
    if (score_id.empty()) {
        StatusOr<WorkspaceManifest> ws = WorkspaceManifest::load(ctx.data_root(), workspace.value());
    ...
    if (!ScoreId::from_string(score_id).ok()) {
        return fail(tool, json_mode, ToolErrorCode::Usage, "Unknown score_id: " + score_id, kExitUsage);
    }

    StatusOr<std::string> source = read_all_utf8(input_path.value());
    if (!source.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Io, source.status().message(), kExitUsage);
    }
    StatusOr<std::vector<Index29>> cipher =
        load_input_indices(ctx, source.value(), input_mode);
    if (!cipher.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, cipher.status().message(), kExitFail);
    }

    StatusOr<parcae::tool::TransformEnvelope> envelope =
        parcae::tool::TransformEnvelope::from_json(record.method());
    if (!envelope.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Schema, envelope.status().message(), kExitFail);
    }
    StatusOr<std::vector<Index29>> plain =
        parcae::tool::apply_to_indices(cipher.value(), envelope.value());
    if (!plain.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, plain.status().message(), kExitFail);
    }

    StatusOr<double> value = parcae::tool::score(ctx, plain.value(), score_id);
    if (!value.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, value.status().message(), kExitFail);
    }

    const std::string utc = resolve_utc(args);
    StatusOr<std::string> latin = parcae::tool::to_latin(ctx, plain.value());
    if (!latin.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Internal, latin.status().message(), kExitFail);
    }
    std::size_t max_chars = 64;
    if (record.preview().contains("max_chars") && record.preview().at("max_chars").is_number_unsigned()) {
    ...
    if (!append_ok.ok()) {
        return fail(tool, json_mode, ToolErrorCode::Schema, append_ok.message(), kExitFail);
    }
    ...
    if (record.status() == HypothesisStatus::Draft) {
        ...
        if (!to_proposed.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, to_proposed.message(), kExitFail);
        }
    }
    if (record.status() == HypothesisStatus::Proposed) {
        Status to_scored = record.set_status(HypothesisStatus::Scored);
        if (!to_scored.ok()) {
            return fail(tool, json_mode, ToolErrorCode::Schema, to_scored.message(), kExitFail);
        }
    }
...
[[nodiscard]] int cmd_set_status(
    ...
    StatusOr<std::string> status_text = require_option(args, "--status");
    if (!status_text.ok()) {
