...
TEST_CASE("BenchTimer time_cpu_once counts launches and returns rates", "[bench][timer]") {
    ...
    if (sample.value().wall_seconds() > 0.0) {
        const double expected =
            BenchMetric::runes_per_sec(8, 4, 16, sample.value().wall_seconds());
...
TEST_CASE("BenchTimer time_cpu propagates launch errors", "[bench][timer]") {
    StatusOr<BenchMetric::Sample> sample = BenchTimer::time_cpu(
        ...
        1,
        1,
        []() -> Status { return Status::error("bench boom"); });
...
TEST_CASE("BenchTimer time_cuda empty launches follow median-of-3 protocol", "[bench][timer][cuda]") {
    std::atomic<std::uint32_t> hits{0};
    constexpr std::size_t reps = 2;
    StatusOr<BenchMetric::Sample> sample = BenchTimer::time_cuda(
        reps,
        /*candidates=*/8,
        /*tokens=*/32,
        [&]() -> Status {
