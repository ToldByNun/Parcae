...
TEST_CASE("ParityRecord capture hashes params input output interrupt", "[parity]") {
    ...
    const ParityRecord record =
        ParityRecord::capture("caesar", params, interrupt.value(), input, output, "cpu");
    ...
    REQUIRE(record.interrupt_sha256() == ParityRecord::hash_json(interrupt.value().to_json()));
...
TEST_CASE("ParityRecord JSON round-trip", "[parity]") {
    ...
    const ParityRecord original = ParityRecord::capture("identity", nlohmann::json::object(),
                                                        InterruptPolicy::none(), xs, xs);
...
TEST_CASE("ParityRecord apply_and_capture matches ApplyTransform", "[parity]") {
    ...
    StatusOr<std::pair<std::vector<Index29>, ParityRecord>> ran = ParityRecord::apply_and_capture(
        TransformId::caesar(), plain, params, TransformDirection::Encrypt);
    REQUIRE(ran.ok());

    StatusOr<std::vector<Index29>> direct =
        ApplyTransform::apply(TransformId::caesar(), plain, params, TransformDirection::Encrypt);
    REQUIRE(direct.ok());
...
TEST_CASE("ParityRecord distinguishes different outputs", "[parity]") {
    ...
    const auto a = ParityRecord::capture("atbash", nlohmann::json::object(),
                                         InterruptPolicy::none(), input, input);
    const std::vector<Index29> flipped{Index29{28}, Index29{27}};
    const auto b = ParityRecord::capture("atbash", nlohmann::json::object(),
                                         InterruptPolicy::none(), input, flipped);
