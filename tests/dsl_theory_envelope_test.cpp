...
[[nodiscard]] StatusOr<TheoryArtifact> make_poly2_artifact() {
    ...
    return TheoryArtifact::make(
        "quadratic_polynomial_stream", 1, TheoryIr::Tier::B, TheoryIr::Family::KeyedStream, kSha,
        ...
        {"poly2_mod29"}, std::string("Speculative. Test claim."), std::nullopt, std::move(paths));
}
...
TEST_CASE("TheoryEnvelopeBridge parses theory URI extension", "[dsl][envelope]") {
    ...
    REQUIRE(env.value().theory_uri()->to_string() ==
...
TEST_CASE("TheoryEnvelopeBridge::template_for writes round-trip file", "[dsl][envelope]") {
    ...
    StatusOr<TheoryEnvelopeBridge::Envelope> tmpl = TheoryEnvelopeBridge::template_for(art.value());
    REQUIRE(tmpl.ok());
    ...
    const auto dir = std::filesystem::temp_directory_path() / "parcae_theory_envelope_h38";
    std::error_code ec;
...
TEST_CASE("TheoryEnvelopeBridge::check_against_artifact rejects URI mismatch", "[dsl][envelope]") {
    StatusOr<TheoryArtifact> art = make_poly2_artifact();
    REQUIRE(art.ok());
...
TEST_CASE("TheoryEnvelopeBridge::check_against_artifact rejects out-of-domain param",
          "[dsl][envelope]") {
    StatusOr<TheoryArtifact> art = make_poly2_artifact();
    REQUIRE(art.ok());
...
TEST_CASE("catalog TransformEnvelope still parses via tool API", "[dsl][envelope]") {
    ...
    StatusOr<TransformEnvelope> direct = TransformEnvelope::from_json(root);
    REQUIRE(direct.ok());

    StatusOr<TheoryEnvelopeBridge::Envelope> via_bridge = TheoryEnvelopeBridge::from_json(root);
    REQUIRE(via_bridge.ok());
    StatusOr<TransformEnvelope> lowered = via_bridge.value().to_catalog_envelope();
    REQUIRE(lowered.ok());
