...
[[nodiscard]] TransformCandidate caesar_candidate(std::uint8_t shift) {
    return TransformCandidate(
        "caesar:shift=" + std::to_string(shift), TransformId::caesar(), TransformDirection::Decrypt,
        nlohmann::json{{"shift", static_cast<int>(shift)}}, {Index29{19}, Index29{7}, Index29{4}});
}

[[nodiscard]] StatusOr<BatchArtifact> make_test_batch(std::string_view workspace_id,
    ...
    StatusOr<SearchPrior> prior = SearchPrior::make(workspace_id, {}, {}, "2026-09-21T18:00:00Z");
    if (!prior.ok()) {
        return prior.status();
    }
    return BatchArtifact::make(workspace_id, batch_id, "2026-09-21T18:00:01Z",
...
TEST_CASE("HypothesisBridge ingest creates proposed records from batch top-k", "[search][bridge]") {
    const auto tmp = std::filesystem::temp_directory_path() / "parcae_hypothesis_bridge_f19";
    std::error_code ec;
    ...
    StatusOr<BatchArtifact> art = make_test_batch("bridge-ws", "b-caesar-0001", std::move(lines));
    REQUIRE(art.ok());
    REQUIRE(art.value().store(tmp).ok());

    StatusOr<HypothesisBridge::Result> ingested = HypothesisBridge::ingest(tmp, art.value());
    REQUIRE(ingested.ok());
    ...
    const std::string id0 =
        HypothesisBridge::hypothesis_id_for("bridge-ws", "b-caesar-0001", "caesar:shift=3");
    const std::string id1 =
        HypothesisBridge::hypothesis_id_for("bridge-ws", "b-caesar-0001", "caesar:shift=7");
    REQUIRE(id0 != id1);
    REQUIRE(id0.size() == 33);
    REQUIRE(id0[0] == 'h');
    ...
    StatusOr<std::vector<std::string>> listed = HypothesisRecord::list_ids(tmp, "bridge-ws");
    REQUIRE(listed.ok());
...
TEST_CASE("HypothesisBridge overwrite_existing=false skips existing ids", "[search][bridge]") {
    const auto tmp = std::filesystem::temp_directory_path() / "parcae_hypothesis_bridge_f19_skip";
    std::error_code ec;
    std::filesystem::remove_all(tmp, ec);
    std::filesystem::create_directories(tmp / "workspaces", ec);
    ...
    StatusOr<HypothesisBridge::Result> skipped = HypothesisBridge::ingest(tmp, art.value(), opts);
    REQUIRE(skipped.ok());
...
TEST_CASE("HypothesisBridge does not auto-promote; preserves scored status on refresh",
    ...
    const std::string id =
        HypothesisBridge::hypothesis_id_for("bridge-ws-scored", "b-scored-0001", "caesar:shift=3");
    ...
    StatusOr<HypothesisBridge::Result> refreshed = HypothesisBridge::ingest(tmp, art.value());
    REQUIRE(refreshed.ok());
    REQUIRE(refreshed.value().updated_ids().size() == 1);

    StatusOr<HypothesisRecord> again = HypothesisRecord::load(tmp, "bridge-ws-scored", id);
    REQUIRE(again.ok());
...
TEST_CASE("HypothesisBridge id is pure over workspace/batch/candidate triple",
    ...
    REQUIRE(pre == "ws-a\nb-1\ncaesar:shift=3");
    REQUIRE(a == std::string("h") +
                     Sha256::hex_digest(pre).substr(0, HypothesisBridge::id_digest_hex_len));

    // Any field change flips the id.
    REQUIRE(HypothesisBridge::hypothesis_id_for("ws-b", "b-1", "caesar:shift=3") != a);
    REQUIRE(HypothesisBridge::hypothesis_id_for("ws-a", "b-2", "caesar:shift=3") != a);
    REQUIRE(HypothesisBridge::hypothesis_id_for("ws-a", "b-1", "caesar:shift=4") != a);
}

TEST_CASE("HypothesisBridge different batches stay separate files for same candidate_id",
          "[search][bridge][idempotent]") {
    const auto tmp =
    ...
    auto line = [&](std::size_t rank) {
        return BatchArtifact::candidate_wire(caesar_candidate(3), "chi2_english_gp_v0", "v0", 1.0,
                                             Backend::Cpu, rank);
    };

    StatusOr<BatchArtifact> b1 = make_test_batch("bridge-ws-f20", "b-run-0001", {line(0)});
    StatusOr<BatchArtifact> b2 = make_test_batch("bridge-ws-f20", "b-run-0002", {line(0)});
    REQUIRE(b1.ok());
    REQUIRE(b2.ok());
    REQUIRE(HypothesisBridge::ingest(tmp, b1.value()).ok());
    REQUIRE(HypothesisBridge::ingest(tmp, b2.value()).ok());

    StatusOr<HypothesisRecord> from_b1 =
        HypothesisBridge::load_for_candidate(tmp, "bridge-ws-f20", "b-run-0001", "caesar:shift=3");
    StatusOr<HypothesisRecord> from_b2 =
        HypothesisBridge::load_for_candidate(tmp, "bridge-ws-f20", "b-run-0002", "caesar:shift=3");
    REQUIRE(from_b1.ok());
    REQUIRE(from_b2.ok());
    ...
    StatusOr<std::vector<std::string>> listed = HypothesisRecord::list_ids(tmp, "bridge-ws-f20");
    REQUIRE(listed.ok());
    REQUIRE(listed.value().size() == 2);
...
TEST_CASE("HypothesisBridge make_batch_source + require_batch_provenance",
          "[search][bridge][idempotent]") {
    const nlohmann::json ok = HypothesisBridge::make_batch_source("b-ok-0001", "caesar:shift=3",
                                                                  "caesar", "gen_caesar", 2);
    ...
    const nlohmann::json with_agent = HypothesisBridge::make_batch_source(
        "b-ok-0001", "caesar:shift=3", "caesar", "gen_caesar", 0, std::string("a1b2c3d4"));
    ...
    REQUIRE_FALSE(HypothesisBridge::require_batch_provenance(
                      ...
                      .ok());
    REQUIRE_FALSE(HypothesisBridge::require_batch_provenance(nlohmann::json{
                                                                 {"batch_id", "BadBatch"},
                                                                 {"candidate_id", "x"},
                                                             })
                      .ok());
