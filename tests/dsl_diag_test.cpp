...
TEST_CASE("DslDiag formats path:line:col: RULE message", "[dsl][diag]") {
    const DslDiag d = DslDiag::make(DslRuleId::E013_tier_structural_claim,
                                    "tier B requires structural_claim()", "theories/x.py", 42, 4);
...
TEST_CASE("DslDiag formats import whitelist example", "[dsl][diag]") {
    const DslDiag d =
        DslDiag::make(DslRuleId::E021_import_whitelist,
                      "import 'numpy' not in parcae.dsl.* whitelist", "theories/x.py", 3, 1);
...
TEST_CASE("DslDiag formats interrupt example with hint", "[dsl][diag]") {
    const DslDiag d =
        DslDiag::make(DslRuleId::E030_interrupt_policy, "interrupt_policy missing", "theories/x.py",
                      88, 4, "set interrupts='none_by_design' if intentional");
    REQUIRE(d.format() == "theories/x.py:88:4: E030 interrupt_policy missing");
    REQUIRE(d.format_with_hint() == "theories/x.py:88:4: E030 interrupt_policy missing\n"
...
TEST_CASE("DslDiag lineno without path", "[dsl][diag]") {
    const DslDiag d = DslDiag::make(DslRuleId::E031_forbidden_construct,
                                    "for-loop not allowed in primitive body", {}, 10, 0);
...
TEST_CASE("DslDiag missing col defaults to 0 when lineno set", "[dsl][diag]") {
    const DslDiag d = DslDiag::make(DslRuleId::E100_json_schema, "missing schema field",
                                    "dump.json", 1, std::nullopt);
