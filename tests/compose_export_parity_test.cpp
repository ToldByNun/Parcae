...
void require_same_topk(const CpuCandidateExport::Result& cpu,
    ...
    for (std::size_t i = 0; i < cpu.size(); ++i) {
        ...
        REQUIRE(cpu.rows()[i].candidate().transform_id() == TransformId::compose());
        REQUIRE(gpu.rows()[i].candidate().transform_id() == TransformId::compose());
    }
}
...
TEST_CASE("Compose job CPU smoke: default grid and explicit recipes",
    ...
    StatusOr<SearchJob> default_job =
        SearchJob::make("_example", "compose", "chi2_english_gp_v0", k, 1, Backend::Cpu, 64);
    ...
    StatusOr<CpuCandidateExport::Result> atbash_caesar =
    ...
    REQUIRE(recipes.value().rows()[0].candidate().candidate_id().find("atbash_caesar:shift=") !=
            std::string::npos);
}
...
TEST_CASE("Compose job CPU vs CUDA mirror smoke (AtbashCaesar grid + ComposeDriver recipes)",
    ...
    // Default compose grid ↔ fused Atbash∘Caesar export.
    {
        ...
        StatusOr<CpuCandidateExport::Result> cpu =
            CpuCandidateExport::run(cipher, "compose", "chi2_english_gp_v0", k, ctx);
        REQUIRE(cpu.ok());

        StatusOr<GpuCandidateExport::Result> gpu_compose =
            GpuCandidateExport::compose_from_param_grid(cipher, freqs.value(),
                                                        nlohmann::json::object(), k);
    ...
    // Explicit short recipe list ↔ ComposeDriver apply + host χ².
    {
        ...
        const nlohmann::json grid = two_atbash_caesar_recipes();
        REQUIRE_FALSE(ComposeRecipeCandidateGenerator::is_full_atbash_caesar_grid(
        ...
        StatusOr<CpuCandidateExport::Result> cpu = CpuCandidateExport::run(
            cipher, "compose", "chi2_english_gp_v0", k, ctx, TransformDirection::Decrypt, grid);
        REQUIRE(cpu.ok());
        REQUIRE(cpu.value().size() == k);

        StatusOr<GpuCandidateExport::Result> gpu =
            GpuCandidateExport::compose_from_param_grid(cipher, freqs.value(), grid, k);
        REQUIRE(gpu.ok());
    ...
    // scheduler uses GpuCandidateExport — mirror via compose_from_param_grid above.
    {
        StatusOr<SearchJob> job =
            SearchJob::make("_example", "compose", "chi2_english_gp_v0", 3, 1, Backend::Cuda, 64);
        REQUIRE(job.ok());
        StatusOr<GpuCandidateExport::Result> gpu = GpuCandidateExport::compose_from_param_grid(
...
TEST_CASE("Compose job CUDA mirror skipped (PARCAE_HAS_CUDA unset)",
          "[search][export][compose][parity][cuda]") {
    SUCCEED("PARCAE_HAS_CUDA unset — CPU compose smoke covered above; CUDA mirror not linked");
}
