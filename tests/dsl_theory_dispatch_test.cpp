...
TEST_CASE("TheoryApplyIr round-trips poly2 TheoryIr", "[dsl][dispatch][i41]") {
    ...
    StatusOr<TheoryIr> th =
        TheoryIr::make("caesar_rt", TheoryIr::Family::Elementwise, TheoryIr::Tier::A,
    ...
    StatusOr<std::vector<Index29>> via_ir =
        TheoryDispatch::apply(back.value(), input, params, TransformDirection::Encrypt);
    REQUIRE(via_ir.ok());
    StatusOr<std::vector<Index29>> direct =
        DslIrApplicator::apply(th.value(), input, params, TransformDirection::Encrypt);
    REQUIRE(direct.ok());
...
TEST_CASE("TheoryDispatch catalog envelope delegates to ApplyTransform", "[dsl][dispatch][i41]") {
    ...
    StatusOr<TheoryEnvelopeBridge::Envelope> env = TheoryEnvelopeBridge::from_json(env_json);
    REQUIRE(env.ok());
    REQUIRE(env.value().is_catalog());

    const auto input = stream_of({0, 1, 2});
    StatusOr<std::vector<Index29>> out = TheoryDispatch::apply(root, env.value(), input);
    REQUIRE(out.ok());
...
TEST_CASE("TheoryDispatch applies compiled new_math theory URI", "[dsl][dispatch][i41][compile]") {
    REQUIRE(DslCompile::pipeline_ready(compile_options()));
    const auto root = std::filesystem::temp_directory_path() / "parcae_dispatch_new_math_i41";
    std::error_code ec;
    std::filesystem::remove_all(root, ec);
    std::filesystem::create_directories(root, ec);

    const auto src = std::filesystem::path(PARCAE_EXAMPLES_DIR) / "new_math_example.py";
    StatusOr<DslCompile::Result> compiled = DslCompile::compile_file(src, root, compile_options());
    REQUIRE(compiled.ok());
    ...
    params["c1"] = 0;
    params["c0"] = 0;
    TheoryEnvelopeBridge::Envelope bound{TheoryEnvelopeBridge::Kind::Theory,
    ...
    TheoryEnvelopeBridge::Envelope dec{TheoryEnvelopeBridge::Kind::Theory,
                                       bound.transform_id(),
                                       TransformDirection::Decrypt,
                                       bound.params(),
                                       bound.interrupt(),
                                       bound.theory_uri()};
    StatusOr<std::vector<Index29>> plain = TheoryDispatch::apply(root, dec, out.value());
    REQUIRE(plain.ok());
...
TEST_CASE("TheoryDispatch applies compiled koan1_style compose URI",
          "[dsl][dispatch][i41][compile]") {
    REQUIRE(DslCompile::pipeline_ready(compile_options()));
    const auto root = std::filesystem::temp_directory_path() / "parcae_dispatch_koan_i41";
    std::error_code ec;
    std::filesystem::remove_all(root, ec);
    std::filesystem::create_directories(root, ec);

    const auto src = std::filesystem::path(PARCAE_EXAMPLES_DIR) / "full_lifecycle_example.py";
    StatusOr<DslCompile::Result> compiled = DslCompile::compile_file(src, root, compile_options());
    REQUIRE(compiled.ok());

    StatusOr<TheoryEnvelopeBridge::Envelope> env =
        TheoryEnvelopeBridge::load(root / "koan1_style" / "1" / "envelope.json");
    REQUIRE(env.ok());
    nlohmann::json params = env.value().params();
    params["caesar_shift"] = 7;
    TheoryEnvelopeBridge::Envelope enc{TheoryEnvelopeBridge::Kind::Theory,
                                       env.value().transform_id(),
                                       TransformDirection::Encrypt,
                                       params,
                                       env.value().interrupt(),
    ...
    REQUIRE(cipher.ok());
    StatusOr<std::vector<Index29>> plain = TheoryDispatch::apply(root, dec, cipher.value());
    REQUIRE(plain.ok());
    REQUIRE(plain.value() == input);
