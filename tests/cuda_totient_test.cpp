...
[[nodiscard]] StatusOr<std::vector<std::uint8_t>> host_shifts(
    std::size_t consumable,
...
TEST_CASE("CUDA totient parity hand vectors with and without interrupts", "[cuda][parity][totient]") {
    ...
    SECTION("no interrupts") {
        ...
        REQUIRE(TotientPrimeStreamTransform::kernel(
                    plain, cpu_out, cpu_shifts, {}, TransformDirection::Encrypt)
                    .ok());

        std::vector<std::uint8_t> host_out(host_in.size(), 0xFFu);
        REQUIRE(TotientPrimeStreamKernel::apply_host(
                    host_in, host_out, shifts.value(), view.value(), CudaDir::Encrypt)
                    .ok());
        ...
        REQUIRE(TotientPrimeStreamKernel::apply_host(
                    host_out, recovered, shifts.value(), view.value(), CudaDir::Decrypt)
                    .ok());
...
TEST_CASE("CUDA totient random and sorted-skips encoding", "[cuda][parity][totient]") {
    ...
    std::vector<std::uint8_t> host_out(host_in.size());
    REQUIRE(TotientPrimeStreamKernel::apply_host(
                host_in, host_out, shifts.value(), view.value(), CudaDir::Decrypt)
                .ok());
    REQUIRE(from_bytes(host_out) == cpu_out);
    ...
    REQUIRE(TotientPrimeStreamKernel::apply_host(
                large_in, large_out, large_shifts.value(), large_view.value(), CudaDir::Encrypt)
                .ok());
...
TEST_CASE("CUDA totient rejects wrong shifts length", "[cuda][parity][totient]") {
    ...
    REQUIRE_FALSE(TotientPrimeStreamKernel::apply_host(
                      in, out, bad_shifts, view.value(), CudaDir::Encrypt)
                      .ok());
}
