...
TEST_CASE("PrimitiveIr parses signature and evals poly2", "[dsl][ir]") {
    ...
    const std::vector<Index29> args{Index29{7}, Index29{3}, Index29{5}, Index29{2}};
    const Index29 expect = Z29::add(Z29::add(Z29::mul(Z29::mul(Index29{3}, Index29{7}), Index29{7}),
...
TEST_CASE("PrimitiveIr rejects bad signature", "[dsl][ir]") {
    const StatusOr<PrimitiveIr> prim =
        PrimitiveIr::make("bad", "(i: int) -> Z29", Z29Expr::var("i"));
...
TEST_CASE("TheoryIr elementwise tier A ok without claim", "[dsl][ir]") {
    ...
    const StatusOr<TheoryIr> th =
        TheoryIr::make("my_affine", TheoryIr::Family::Elementwise, TheoryIr::Tier::A,
...
TEST_CASE("TheoryIr tier B requires structural_claim E013", "[dsl][ir]") {
    const StatusOr<TheoryIr> th =
        TheoryIr::make("speculative", TheoryIr::Family::Elementwise, TheoryIr::Tier::B,
                       TheoryIr::InterruptMode::ElementwiseDefault, {}, {}, {}, std::nullopt,
                       "theories/x.py", 42, 4);
    REQUIRE_FALSE(th.ok());
...
TEST_CASE("TheoryIr keyed_stream without interrupt is E030", "[dsl][ir]") {
    const StatusOr<TheoryIr> th =
        TheoryIr::make("streamy", TheoryIr::Family::KeyedStream, TheoryIr::Tier::A,
                       TheoryIr::InterruptMode::ElementwiseDefault, {}, {}, {}, std::nullopt,
                       "theories/x.py", 88, 4);
    REQUIRE_FALSE(th.ok());
...
TEST_CASE("TheoryIr keyed_stream with none_by_design ok", "[dsl][ir]") {
    ...
    REQUIRE(th.ok());
    REQUIRE(TheoryIr::interrupt_mode_str(th.value().interrupt_mode()) ==
...
TEST_CASE("ComposeIr koan1-style steps", "[dsl][ir]") {
    ...
    REQUIRE(shift.ok());
    const StatusOr<ComposeIr> comp =
        ComposeIr::make("my_koan1_style", TheoryIr::Tier::A, {"atbash", "caesar"}, {shift.value()},
...
TEST_CASE("ComposeIr rejects unknown step in step_params", "[dsl][ir]") {
    const StatusOr<ComposeIr> comp =
        ComposeIr::make("bad", TheoryIr::Tier::A, {"atbash"}, {},
                        {ComposeIr::StepParamBinding{"caesar", "shift", "caesar_shift"}});
...
TEST_CASE("ComposeIr tier C requires claim", "[dsl][ir]") {
    const StatusOr<ComposeIr> comp = ComposeIr::make(
        "c_compose", TheoryIr::Tier::C, {"atbash", "caesar"}, {}, {}, std::nullopt, "t.py", 1, 0);
    REQUIRE_FALSE(comp.ok());
