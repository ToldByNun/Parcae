...
TEST_CASE(
    ...
    {
        StatusOr<CpuCandidateExport::Result> atbash = CpuCandidateExport::run(
...

    {
        ...
        StatusOr<CpuCandidateExport::Result> caesar = CpuCandidateExport::run(
            cipher, "caesar", "chi2_english_gp_v0", k, ctx);
        REQUIRE(caesar.ok());
    ...
    {
        constexpr std::size_t k = 5;
        StatusOr<CpuCandidateExport::Result> composed = CpuCandidateExport::run(
...
TEST_CASE(
    "CpuCandidateExport vs GpuCandidateExport top-k parity on a-warning",
...
TEST_CASE(
    "Cpu vs Gpu export parity device path skipped (PARCAE_HAS_CUDA unset)",
    "[search][export][parity][a-warning][cuda]") {
