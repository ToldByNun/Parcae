...
TEST_CASE(
    "DslFuse koan1 fused decrypt matches ComposeTransform golden vector",
    ...
    REQUIRE(DslIrApplicator::apply_into(
                fused.value().theory().decrypt_step(), "x", env, cipher, out)
                .ok());
    REQUIRE(out == expect);

    StatusOr<std::vector<Index29>> ref = ComposeTransform::apply_atbash_then_caesar(
        cipher, 3, TransformDirection::Decrypt);
    REQUIRE(ref.ok());
    REQUIRE(out == ref.value());
}

TEST_CASE(
    "DslFuse koan1 fused encrypt/decrypt roundtrip matches ComposeTransform",
    "[dsl][fuse][koan][parity]") {
    const TheoryIr atbash = make_atbash();
    const TheoryIr caesar = make_caesar();
    const ComposeIr compose = make_koan1_compose();
    ...
    REQUIRE(DslIrApplicator::apply_into(
                fused.value().theory().encrypt_step(), "x", env, plain, cipher)
                .ok());

    StatusOr<std::vector<Index29>> ref_cipher = ComposeTransform::apply_atbash_then_caesar(
        plain, shift, TransformDirection::Encrypt);
    REQUIRE(ref_cipher.ok());
    REQUIRE(cipher == ref_cipher.value());

    std::vector<Index29> roundtrip(cipher.size());
    REQUIRE(DslIrApplicator::apply_into(
                fused.value().theory().decrypt_step(), "x", env, cipher, roundtrip)
                .ok());
    REQUIRE(roundtrip == plain);

    StatusOr<std::vector<Index29>> ref_plain = ComposeTransform::apply_atbash_then_caesar(
...
TEST_CASE(
    "DslFuse koan1 fused matches ComposeTransform for all shifts 0..28",
    "[dsl][fuse][koan][parity]") {
    const TheoryIr atbash = make_atbash();
    const TheoryIr caesar = make_caesar();
    const ComposeIr compose = make_koan1_compose();
    ...
    const std::vector<Index29> stream{
        Index29{0}, Index29{1}, Index29{10}, Index29{15}, Index29{28}};

    for (std::uint8_t shift = 0; shift < Index29::modulus; ++shift) {
        Z29Expr::Env env{{"caesar_shift", Index29{shift}}};
        std::vector<Index29> fused_out(stream.size());
        REQUIRE(DslIrApplicator::apply_into(
                    fused.value().theory().decrypt_step(), "x", env, stream, fused_out)
                    .ok());

        StatusOr<std::vector<Index29>> ref = ComposeTransform::apply_atbash_then_caesar(
            stream, shift, TransformDirection::Decrypt);
        REQUIRE(ref.ok());
...
TEST_CASE(
    "DslFuse koan1 staged recipe includes caesar encrypt direction",
    "[dsl][fuse][koan][parity]") {
    const TheoryIr atbash = make_atbash();
    const TheoryIr caesar = make_caesar();
    const ComposeIr compose = make_koan1_compose();
    ...
    const std::vector<Index29> cipher{Index29{0}, Index29{5}, Index29{28}};
    StatusOr<std::vector<Index29>> via_recipe = ComposeTransform{}.apply(
    ...
    REQUIRE(DslIrApplicator::apply_into(
                fused.value().theory().decrypt_step(), "x", env, cipher, fused_out)
                .ok());
    REQUIRE(via_recipe.value() == fused_out);
}

TEST_CASE(
    "DslFuse koan1 staged cuda façade sets CudaDir::Encrypt on caesar",
    "[dsl][fuse][koan]") {
    const TheoryIr atbash = make_atbash();
    const TheoryIr caesar = make_caesar();
    const ComposeIr compose = make_koan1_compose();
    ...
    REQUIRE(bundle.ok());
    REQUIRE(
        bundle.value().staged_cuda_header().find("CudaFamilyId::Caesar") != std::string::npos);
    REQUIRE(
        bundle.value().staged_cuda_header().find("CudaDir::Encrypt") != std::string::npos);
}
