...
TEST_CASE(
    "Committed stale-major fixture is rejected by TheoryRegistry",
    "[dsl][registry][stale]") {
    ...
    const std::filesystem::path manifest =
        root / "stale_major_demo" / "1" / "manifest.json";
    REQUIRE(std::filesystem::is_regular_file(manifest));

    // Bytes are a valid ready-shaped artifact for an older MAJOR.
    StatusOr<TheoryArtifact> raw =
        TheoryArtifact::load(root, "stale_major_demo", 1);
    REQUIRE(raw.ok());
    ...
    StatusOr<TheoryArtifact> gated =
        TheoryRegistry::load(root, "stale_major_demo", 1);
    ...
    StatusOr<TheoryArtifact> by_uri = TheoryRegistry::load_uri(
        root, "parcae://theories/stale_major_demo@1");
    REQUIRE_FALSE(by_uri.ok());
}

TEST_CASE(
    "Catalog lists stale-major fixture with stale_spec and not ready",
    "[dsl][registry][stale]") {
...
TEST_CASE(
    "Simulated MAJOR bump invalidates today's 1.0.0 artifact bytes",
    "[dsl][registry][stale]") {
    ...
    const auto root =
        std::filesystem::temp_directory_path() / "parcae_dsl_major_bump_h33";
    std::error_code ec;
    ...
    REQUIRE(made.value().store(root).ok());
    {
        ...
        std::ofstream out(
            TheoryArtifact::artifact_dir(root, "bump_victim", 1) / "manifest.json",
...
TEST_CASE(
    "Recompile after major bump restores registry load",
    "[dsl][registry][stale]") {
    const auto root =
        std::filesystem::temp_directory_path() / "parcae_dsl_recompile_h33";
    std::error_code ec;
    std::filesystem::remove_all(root, ec);
    std::filesystem::create_directories(root, ec);
    ...
    const std::filesystem::path dst_dir =
        TheoryArtifact::artifact_dir(root, "stale_major_demo", 1);
    std::filesystem::create_directories(dst_dir, ec);
    std::filesystem::copy_file(
        src, dst_dir / "manifest.json", std::filesystem::copy_options::overwrite_existing, ec);
    REQUIRE(!ec);
    ...
    StatusOr<TheoryArtifact> rebuilt = TheoryArtifact::make(
        ...
        {"poly2_mod29"},
        std::nullopt,
        std::string("theories/examples/stale_major_demo.py"));
    REQUIRE(rebuilt.ok());
...
TEST_CASE(
    "Future major on disk is stale and not loadable",
    "[dsl][registry][stale]") {
    StatusOr<TheoryArtifact> made = TheoryArtifact::make(
        "future_major_demo",
        1,
