...
[[nodiscard]] nlohmann::json wire_line(std::uint8_t shift, double score, std::size_t rank,
                                       std::string_view score_id = "chi2_english_gp_v0") {
    return BatchArtifact::candidate_wire(caesar_candidate(shift), score_id, "v0", score,
                                         Backend::Cpu, rank);
}

[[nodiscard]] StatusOr<BatchArtifact> make_batch(std::vector<nlohmann::json> lines, std::size_t k,
                                                 std::string_view score_id = "chi2_english_gp_v0") {
    return BatchArtifact::make("batch-ws", "b-l45-limits", "2026-09-23T07:00:00Z",
                               fake_digest("job-l45"), fake_digest("prior-l45"), "caesar", score_id,
                               "v0", Backend::Cpu, k, 1, std::move(lines));
}

} // namespace

TEST_CASE("L45 BatchArtifact rejects count > k and oversized JSONL lines",
          "[search][batch][limits]") {
    {
        // Three lines but k=2.
    ...
    }
    {
        ...
        const std::size_t n = (BatchArtifact::kMaxCandidateLineBytes / 2) + 64;
    ...
    }
    {
        ...
        const auto tmp = std::filesystem::temp_directory_path() / "parcae_batch_l45_fat_line";
        std::error_code ec;
...
TEST_CASE("L45 BatchArtifact rejects inverted best-first score order",
...
TEST_CASE("L45 BatchOrdering shuffle+sort is deterministic (fuzz)",
    ...
    for (ScoreOrder order : {ScoreOrder::Asc, ScoreOrder::Desc}) {
        for (int trial = 0; trial < 48; ++trial) {
            ...
            hits.reserve(64);
            for (int i = 0; i < 64; ++i) {
                hits.emplace_back("id-" + std::to_string(id_dist(rng)), score_dist(rng),
...
TEST_CASE("L45 BatchArtifact lines from BatchOrdering sort round-trip", "[search][batch][fuzz]") {
    ...
    for (std::uint8_t shift = 0; shift < 16; ++shift) {
        hits.emplace_back("caesar-shift-" + std::to_string(shift), score_dist(rng),
    ...
    if (hits.size() >= 2) {
        ...
        inverted.push_back(
            wire_line(static_cast<std::uint8_t>(hits[1].source_index()), hits[1].score(), 0));
        inverted.push_back(
            wire_line(static_cast<std::uint8_t>(hits[0].source_index()), hits[0].score(), 1));
        // Only fail when the swap actually violates Asc order.
        if (BatchOrdering::better(BatchHit{hits[1].candidate_id(), hits[1].score(), 0},
