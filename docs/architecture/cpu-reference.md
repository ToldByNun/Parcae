...
include/parcae/
  ...
  dsl/           Theory DSL compiler (ingest + gate + IR + CPU applicator; emit later)
...
| `dsl` | Theory DSL compile (ingest + gate + IR + CPU applicator; emit later) | Emitted twins under `cuda/emitted/` |
