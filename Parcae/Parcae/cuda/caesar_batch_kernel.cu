...
__global__ void caesar_batch_shared_kernel(
    ...
    if (directions[candidate] != 0u) {
        out[flat] = static_cast<std::uint8_t>((x + shift) % kModulus);
    } else {
        out[flat] = static_cast<std::uint8_t>((x + kModulus - shift) % kModulus);
    }
}
...
[[nodiscard]] Status validate_batch(
    ...
    if (candidate_count > CandidateBatchBuffers::kMaxC) {
        return Status::error("CaesarBatchKernel: C exceeds kMaxC");
    }
    if (token_count > CandidateBatchBuffers::kMaxT) {
        return Status::error("CaesarBatchKernel: T exceeds kMaxT");
    }
    if (shifts.size() != candidate_count) {
        return Status::error("CaesarBatchKernel: shifts size must equal C");
    }
    if (directions.size() != candidate_count) {
        return Status::error("CaesarBatchKernel: directions size must equal C");
    }
    for (std::size_t c = 0; c < candidate_count; ++c) {
        if (shifts[c] >= kModulus) {
            return Status::error("CaesarBatchKernel: shift must be in 0..28");
        }
        if (directions[c] > 1u) {
            return Status::error("CaesarBatchKernel: direction must be 0 or 1");
        }
    }
...
Status CaesarBatchKernel::launch_device(
    ...
    if (device_in == nullptr || device_shifts == nullptr || device_directions == nullptr ||
        device_out == nullptr) {
        return Status::error("CaesarBatchKernel::launch_device null device pointer");
    }
...
Status CaesarBatchKernel::apply_host(
    ...
    if (out.size() != candidate_count * token_count) {
        return Status::error("CaesarBatchKernel::apply_host out size must be C * T");
    }
    if (token_count == 0) {
        return Status::success();
    }
...
Status CaesarBatchKernel::apply_host(CandidateBatchBuffers& buffers) {
    if (buffers.token_layout() != CandidateBatchBuffers::TokenLayout::Shared) {
        return Status::error("CaesarBatchKernel requires Shared token layout");
    }
