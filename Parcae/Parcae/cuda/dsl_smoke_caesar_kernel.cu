...
__global__ void dsl_smoke_caesar_kernel(const std::uint8_t* in, std::uint8_t* out,
...
Status DslSmokeCaesarKernel::launch_device(const std::uint8_t* device_in, std::uint8_t* device_out,
                                           std::size_t count, std::uint8_t shift,
                                           CudaDir direction) {
    if (shift > 28u) {
    ...
    dsl_smoke_caesar_kernel<<<blocks, kThreadsPerBlock>>>(device_in, device_out, count, shift,
                                                          encrypt);

    Status launch = CudaError::to_status(cudaGetLastError(), "DslSmokeCaesarKernel::launch_device");
    if (!launch.ok()) {
        return launch;
    }
    return CudaError::to_status(cudaDeviceSynchronize(),
                                "DslSmokeCaesarKernel::launch_device sync");
}

Status DslSmokeCaesarKernel::apply_host(std::span<const std::uint8_t> host_in,
