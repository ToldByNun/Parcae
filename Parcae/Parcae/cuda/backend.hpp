...
public:
    ...
    apply_and_capture(const TransformId& id, std::span<const Index29> input,
        ...
        ParityRecord record =
            ParityRecord::capture(id.str(), params, interrupt, input, output.value(), "cuda");
...
private:
    ...
    [[nodiscard]] static Status write_indices(std::span<const std::uint8_t> bytes,
    ...
    [[nodiscard]] static Status apply_identity(std::span<const Index29> input,
                                               std::span<Index29> output) {
    ...
    [[nodiscard]] static Status apply_atbash(std::span<const Index29> input,
                                             std::span<Index29> output) {
        std::vector<std::uint8_t> bytes = to_bytes(input);
    ...
    [[nodiscard]] static Status apply_caesar(std::span<const Index29> input,
                                             std::span<Index29> output, const CaesarParams& params,
                                             CudaDir direction) {
        std::vector<std::uint8_t> bytes = to_bytes(input);
    ...
    [[nodiscard]] static Status apply_affine(std::span<const Index29> input,
                                             std::span<Index29> output, const AffineParams& params,
                                             CudaDir direction) {
        std::vector<std::uint8_t> bytes = to_bytes(input);
        Status status = AffineKernel::apply_host(bytes, bytes, params.a, params.b, direction);
        if (!status.ok()) {
            return status;
        }
        return write_indices(bytes, output);
    ...
    [[nodiscard]] static Status apply_vigenere(std::span<const Index29> input,
                                               std::span<Index29> output, const KeyParamsHost& key,
                                               const InterruptDeviceView& interrupts,
                                               CudaDir direction) {
        std::vector<std::uint8_t> bytes = to_bytes(input);
        Status status = VigenereKeyKernel::apply_host(bytes, bytes, key.key, interrupts, direction);
        if (!status.ok()) {
            return status;
        }
        return write_indices(bytes, output);
    ...
    [[nodiscard]] static Status apply_beaufort(std::span<const Index29> input,
                                               std::span<Index29> output, const KeyParamsHost& key,
                                               const InterruptDeviceView& interrupts) {
        std::vector<std::uint8_t> bytes = to_bytes(input);
    ...
    [[nodiscard]] static Status apply_totient(std::span<const Index29> input,
                                              std::span<Index29> output,
        ...
        std::vector<std::uint8_t> bytes = to_bytes(input);
        Status status =
            TotientPrimeStreamKernel::apply_host(bytes, bytes, shift_bytes, interrupts, direction);
        if (!status.ok()) {
            return status;
        }
        return write_indices(bytes, output);
    ...
    [[nodiscard]] static Status apply_compose(std::span<const Index29> input,
                                              std::span<Index29> output,
                                              const ComposeParamsHost& recipe,
                                              const InterruptDeviceView& interrupts,
                                              CudaDir direction) {
        std::vector<std::uint8_t> bytes = to_bytes(input);
        Status status = ComposeDriver::apply_host(bytes, bytes, recipe, interrupts, direction);
        if (!status.ok()) {
            return status;
        }
        return write_indices(bytes, output);
    ...
    [[nodiscard]] static Status prepare_params(CudaFamilyId family, const nlohmann::json& params) {
        switch (family) {
        case CudaFamilyId::Identity:
        case CudaFamilyId::Atbash: {
