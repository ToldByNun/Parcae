...
static __global__ void hamming_match_kernel(const std::uint8_t* candidate,
...
Status HammingAgreementScore::count_matches_device(const std::uint8_t* device_candidate,
    ...
    const int blocks = static_cast<int>((count + static_cast<std::size_t>(kHammingThreads) - 1u) /
                                        static_cast<std::size_t>(kHammingThreads));
    hamming_match_kernel<<<blocks, kHammingThreads>>>(device_candidate, device_reference, count,
                                                      device_match_count);
    ...
    return CudaError::to_status(cudaDeviceSynchronize(),
                                "HammingAgreementScore::count_matches_device sync");
}

StatusOr<double> HammingAgreementScore::score_host(std::span<const std::uint8_t> candidate,
    ...
    Status counted = count_matches_device(device_cand.value().data(), device_ref.value().data(), n,
    ...
    Status copied = device_count.value().copy_to_host(std::span<unsigned long long>(&matches, 1));
    if (!copied.ok()) {
        return copied;
    }
