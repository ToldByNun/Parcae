    ...
    generate(std::span<const Index29> ciphertext, const std::vector<ExplicitVigenereKey>& keys,
        ...
        for (std::size_t i = 0; i < keys.size(); ++i) {
            ...
            if (key.key_indices.empty()) {
                return Status::error("gen_vigenere_explicit_keys key_indices must be non-empty");
            }
            ...
            out.emplace_back(make_candidate_id(key.key_indices, i), TransformId::vigenere_key(),
