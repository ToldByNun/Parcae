...
public:
    ...
    /// `workspace_id + "\\n" + batch_id + "\\n" + candidate_id`.
    [[nodiscard]] static std::string id_preimage(std::string_view workspace_id,
                                                 std::string_view batch_id,
                                                 std::string_view candidate_id) {
        std::string material;
        material.reserve(workspace_id.size() + batch_id.size() + candidate_id.size() + 2);
    ...
    [[nodiscard]] static std::string hypothesis_id_for(std::string_view workspace_id,
                                                       std::string_view batch_id,
                                                       std::string_view candidate_id) {
        const std::string digest =
            Sha256::hex_digest(id_preimage(workspace_id, batch_id, candidate_id));
    ...
    load_for_candidate(const std::filesystem::path& data_root, std::string_view workspace_id,
                       std::string_view batch_id, std::string_view candidate_id) {
        return HypothesisRecord::load(data_root, workspace_id,
    ...
    make_batch_source(std::string_view batch_id, std::string_view candidate_id,
                      std::string_view family, std::string_view generator_id, std::size_t rank,
    ...
    [[nodiscard]] static Status require_batch_provenance(const nlohmann::json& source) {
        ...
        if (!batch_ok.ok()) {
            return Status::error("HypothesisBridge: source.batch_id invalid: " +
                                 batch_ok.status().message());
        }
    ...
    generator_id_for_family(std::string_view family) {
        ...
        return Status::error("HypothesisBridge: unsupported family for generator_id mapping: " +
                             std::string(family));
    }
    ...
    [[nodiscard]] static StatusOr<Result> ingest(const std::filesystem::path& data_root,
    ...
    [[nodiscard]] static StatusOr<Result> ingest(const std::filesystem::path& data_root,
        ...
        StatusOr<std::string_view> generator_id = generator_id_for_family(artifact.family());
        if (!generator_id.ok()) {
        ...
        for (const nlohmann::json& line : artifact.candidates()) {
            StatusOr<std::string> written =
                ingest_one(data_root, artifact, line, generator_id.value(), options, out);
            if (!written.ok()) {
    ...
    [[nodiscard]] static StatusOr<Result> ingest_batch_dir(const std::filesystem::path& data_root,
    ...
    [[nodiscard]] static StatusOr<Result> ingest_batch_dir(const std::filesystem::path& data_root,
                                                           ...
                                                           std::string_view batch_id,
                                                           const Options& options) {
        StatusOr<BatchArtifact> artifact = BatchArtifact::load(data_root, workspace_id, batch_id);
        if (!artifact.ok()) {
...
private:
    ...
    [[nodiscard]] static StatusOr<std::string> ingest_one(const std::filesystem::path& data_root,
                                                          const BatchArtifact& artifact,
        ...
        const std::string hypothesis_id =
            hypothesis_id_for(artifact.workspace_id(), artifact.batch_id(), candidate_id);

        StatusOr<std::filesystem::path> path =
            WorkspacePaths::hypothesis_file(data_root, artifact.workspace_id(), hypothesis_id);
        if (!path.ok()) {
            return path.status();
        }
        ...
        StatusOr<TransformEnvelope> envelope = TransformEnvelope::from_json(line.at("envelope"));
        if (!envelope.ok()) {
        ...
        nlohmann::json source = make_batch_source(artifact.batch_id(), candidate_id,
                                                  artifact.family(), generator_id, rank);
        ...
        StatusOr<HypothesisRecord> record = HypothesisRecord::make_draft(
            artifact.workspace_id(), hypothesis_id, artifact.created_utc(),
            title_for(artifact.batch_id(), rank, candidate_id), envelope.value().to_json());
        if (!record.ok()) {
        ...
        record.value().set_rationale("Auto-ingested from batch " + artifact.batch_id() +
                                     " by HypothesisBridge");
    ...
    [[nodiscard]] static std::string title_for(std::string_view batch_id, std::size_t rank,
                                               std::string_view candidate_id) {
        return std::string("batch ") + std::string(batch_id) + " rank " + std::to_string(rank) +
               " " + std::string(candidate_id);
    }

    [[nodiscard]] static StatusOr<std::vector<Index29>>
    parse_output_indices(const nlohmann::json& arr) {
