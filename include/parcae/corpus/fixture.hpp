...
public:
    FixtureLiteralRegion(std::string kind, std::string role, std::string value_file,
                         std::string compare)
        : kind_(std::move(kind)), role_(std::move(role)), value_file_(std::move(value_file)),
    ...
    [[nodiscard]] const std::string& value_file() const noexcept { return value_file_; }

    [[nodiscard]] const std::string& compare() const noexcept { return compare_; }
...
public:
    ...
    FixtureHashes(std::optional<std::string> ciphertext_sha256,
...
class Fixture {
public:
    Fixture(std::string id, std::string ciphertext, std::string plaintext, std::string transform_id,
            std::string direction, std::string verification_status, bool recomputed_ok,
            std::vector<std::size_t> skip_indices,
            std::vector<FixtureLiteralRegion> literal_regions, std::optional<std::string> key_latin,
            std::optional<std::vector<int>> key_indices, FixtureHashes hashes,
            nlohmann::json params = nlohmann::json::object())
        : id_(std::move(id)), ciphertext_(std::move(ciphertext)), plaintext_(std::move(plaintext)),
          transform_id_(std::move(transform_id)), direction_(std::move(direction)),
          verification_status_(std::move(verification_status)), recomputed_ok_(recomputed_ok),
          skip_indices_(std::move(skip_indices)), literal_regions_(std::move(literal_regions)),
          key_latin_(std::move(key_latin)), key_indices_(std::move(key_indices)),
          hashes_(std::move(hashes)), params_(std::move(params)) {}

    [[nodiscard]] const std::string& id() const noexcept { return id_; }

    [[nodiscard]] const std::string& ciphertext() const noexcept { return ciphertext_; }
    ...
    [[nodiscard]] bool recomputed_ok() const noexcept { return recomputed_ok_; }
    ...
    [[nodiscard]] const FixtureHashes& hashes() const noexcept { return hashes_; }
