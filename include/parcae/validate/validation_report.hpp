...
public:
    ...
    [[nodiscard]] const std::vector<ValidationCheck>& checks() const noexcept { return checks_; }

    [[nodiscard]] const std::optional<std::string>& diff_excerpt() const noexcept {
        return diff_excerpt_;
    }

    void set_fixture_id(std::string id) { fixture_id_ = std::move(id); }
    ...
    void set_diff_excerpt(std::string excerpt) { diff_excerpt_ = std::move(excerpt); }
