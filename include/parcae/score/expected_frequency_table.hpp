...
public:
    ...
    ExpectedFrequencyTable(
        std::string id,
        std::array<double, alphabet_size> probabilities,
        ...
        : id_(std::move(id)),
          ...
          raw_counts_(raw_counts),
          raw_total_(raw_total),
          smoothing_(std::move(smoothing)),
    ...
    [[nodiscard]] std::uint64_t raw_total() const noexcept {
        return raw_total_;
    }

    [[nodiscard]] const std::string& smoothing() const noexcept {
        return smoothing_;
    }
    ...
    [[nodiscard]] static StatusOr<ExpectedFrequencyTable> from_raw_counts(
        std::string id,
        const std::array<std::uint64_t, alphabet_size>& raw_counts,
        std::vector<std::string> source_fixture_ids) {
        ...
        const double denom =
            static_cast<double>(total) + static_cast<double>(alphabet_size);
