...
public:
    ...
    ScoreCatalogEntry(std::string id, std::string version, ScoreOrder order, Arity arity)
        : id_(std::move(id)), version_(std::move(version)), order_(order), arity_(arity) {}

    [[nodiscard]] const std::string& id() const noexcept { return id_; }

    [[nodiscard]] const std::string& version() const noexcept { return version_; }

    [[nodiscard]] ScoreOrder order() const noexcept { return order_; }

    [[nodiscard]] Arity arity() const noexcept { return arity_; }
