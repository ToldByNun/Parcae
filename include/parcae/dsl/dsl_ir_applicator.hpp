...
public:
    ...
    apply_into(const Z29Expr::Ptr& step, std::string_view cipher_var, const Z29Expr::Env& params,
        ...
        Status range =
            TransformBuffer::validate_interrupt_range(interrupt, input.size(), "DslIrApplicator");
        if (!range.ok()) {
            return range;
        }
    ...
    [[nodiscard]] static Status
    apply_into(const PrimitiveIr& primitive, std::span<const Index29> param_tail,
               std::span<const Index29> input, std::span<Index29> output,
               const InterruptPolicy& interrupt = InterruptPolicy::none()) {
        ...
        return apply_into(primitive.body(), primitive.param_names()[0], fixed, input, output,
                          interrupt);
    }
    ...
    apply(const Z29Expr::Ptr& step, std::string_view cipher_var, const Z29Expr::Env& params,
    ...
    [[nodiscard]] static StatusOr<std::vector<Index29>>
    apply(const PrimitiveIr& primitive, std::span<const Index29> param_tail,
          std::span<const Index29> input,
          const InterruptPolicy& interrupt = InterruptPolicy::none()) {
        std::vector<Index29> out(input.size());
    ...
    apply(const TheoryIr& theory, std::span<const Index29> input, const nlohmann::json& params,
          ...
          std::string_view cipher_var = "x") {
        std::vector<Index29> out(input.size());
        Status st = apply_into(theory, input, out, params, direction, interrupt, cipher_var);
        if (!st.ok()) {
            return st;
        }
        return out;
...
private:
    ...
    [[nodiscard]] static StatusOr<Z29Expr::Env> bind_theory_params(const TheoryIr& theory,
