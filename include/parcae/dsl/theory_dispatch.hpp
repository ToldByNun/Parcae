...
public:
    ...
    [[nodiscard]] static Status apply_into(const std::filesystem::path& theories_root,
        ...
        if (envelope.is_catalog()) {
            ...
            return ApplyTransform::apply_into(catalog.value().transform_id(), input, output,
                                              catalog.value().params(), catalog.value().direction(),
        ...
        return apply_theory_uri(theories_root, *envelope.theory_uri(), input, output,
                                envelope.params(), envelope.direction(), envelope.interrupt());
    }

    [[nodiscard]] static StatusOr<std::vector<Index29>>
    apply(const std::filesystem::path& theories_root,
          const TheoryEnvelopeBridge::Envelope& envelope, std::span<const Index29> input) {
    ...
    apply_into(const TheoryIr& theory, std::span<const Index29> input, std::span<Index29> output,
        ...
        return DslIrApplicator::apply_into(theory, input, output, params, direction, interrupt,
                                           cipher_var);
    }

    [[nodiscard]] static StatusOr<std::vector<Index29>>
    apply(const TheoryIr& theory, std::span<const Index29> input, const nlohmann::json& params,
          TransformDirection direction, const InterruptPolicy& interrupt = InterruptPolicy::none(),
          std::string_view cipher_var = "x") {
        return DslIrApplicator::apply(theory, input, params, direction, interrupt, cipher_var);
    }

    /// Load `paths.apply_ir` for a registered artifact and return runnable TheoryIr.
    [[nodiscard]] static StatusOr<TheoryIr>
    load_apply_ir(const std::filesystem::path& theories_root, const TheoryUri& uri) {
        ...
        if (!art.value().paths().apply_ir().has_value()) {
            return Status::error("TheoryDispatch: artifact missing paths.apply_ir for " +
                                 uri.to_string() + " (recompile with parcae-compile)");
        }
...
private:
    ...
    [[nodiscard]] static Status
    apply_theory_uri(const std::filesystem::path& theories_root, const TheoryUri& uri,
        ...
        return DslIrApplicator::apply_into(theory.value(), input, output, params, direction,
                                           interrupt, cipher.value());
    }
};
