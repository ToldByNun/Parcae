...
public:
    ...
    class Result {
    public:
        Result(TheoryIr theory, std::vector<std::string> flattened_steps, bool nested_flattened,
               std::string cipher_var)
            : theory_(std::move(theory)), flattened_steps_(std::move(flattened_steps)),
              nested_flattened_(nested_flattened), cipher_var_(std::move(cipher_var)) {}
        ...
        [[nodiscard]] bool nested_flattened() const noexcept { return nested_flattened_; }
    ...
    class BenchReport {
    public:
        BenchReport(double fused_elems_per_sec, double staged_elems_per_sec, FusionStatus status,
                    std::size_t stream_len, std::size_t reps, std::string detail)
            : fused_elems_per_sec_(fused_elems_per_sec),
              staged_elems_per_sec_(staged_elems_per_sec), status_(status), stream_len_(stream_len),
              reps_(reps), detail_(std::move(detail)) {}

        [[nodiscard]] double fused_elems_per_sec() const noexcept { return fused_elems_per_sec_; }

        [[nodiscard]] double staged_elems_per_sec() const noexcept { return staged_elems_per_sec_; }

        [[nodiscard]] FusionStatus status() const noexcept { return status_; }

        [[nodiscard]] std::size_t stream_len() const noexcept { return stream_len_; }
    ...
    [[nodiscard]] static FusionStatus choose_status(double fused_elems_per_sec,
    ...
    class EmitBundle {
    public:
        EmitBundle(FusionStatus status, Result fused, std::string fused_cpu_header,
                   std::string fused_cuda_header, std::string fused_cuda_cu,
                   std::string staged_recipe_json, std::string staged_cpu_header,
                   std::string staged_cuda_header, std::optional<BenchReport> bench = std::nullopt)
            : status_(status), fused_(std::move(fused)),
              ...
              staged_cuda_header_(std::move(staged_cuda_header)), bench_(std::move(bench)) {}
        ...
        [[nodiscard]] const Result& fused() const noexcept { return fused_; }
        ...
        [[nodiscard]] const std::string& fused_cuda_cu() const noexcept { return fused_cuda_cu_; }
        ...
        [[nodiscard]] const std::optional<BenchReport>& bench() const noexcept { return bench_; }

        /// Artifact primary CPU text according to `status`.
        [[nodiscard]] const std::string& selected_cpu_header() const noexcept {
            return status_ == FusionStatus::FallbackStaged ? staged_cpu_header_ : fused_cpu_header_;
        }
    ...
    [[nodiscard]] static StatusOr<Result> fuse_inline(const ComposeIr& compose,
        ...
        bool nested = false;
        Status flatten_st =
            flatten_steps(compose, compose_by_name, flat_steps, flat_bindings, recipe_dirs, nested,
                          /*depth=*/0);
        if (!flatten_st.ok()) {
            return flatten_st;
        }
        ...
        for (const std::string& step_id : flat_steps) {
            ...
            StatusOr<Z29Expr::Ptr> next = inline_stage(*th, step_id, flat_bindings, decrypt,
                                                       cipher_var, uses_encrypt_step(recipe));
            if (!next.ok()) {
                return next.status();
            }
            decrypt = next.value();
        ...
        for (std::size_t i = flat_steps.size(); i > 0; --i) {
            ...
            StatusOr<Z29Expr::Ptr> next =
                inline_stage(*th, step_id, flat_bindings, encrypt, cipher_var,
        ...
        StatusOr<TheoryIr> fused =
            TheoryIr::make(compose.name(), TheoryIr::Family::Elementwise, compose.tier(),
                           TheoryIr::InterruptMode::ElementwiseDefault, compose.params(), encrypt,
                           decrypt, compose.structural_claim());
        if (!fused.ok()) {
            return fused.status();
        }

        return Result{fused.value(), std::move(flat_steps), nested, std::string(cipher_var)};
    }

    /// ComposeTransform / ComposeDriver recipe JSON (`{"stages":[…]}`).
    [[nodiscard]] static StatusOr<std::string>
    emit_staged_recipe_json(const ComposeIr& compose, std::span<const TheoryIr> theories,
                            std::span<const ComposeIr> composes = {},
        ...
        for (const std::string& step_id : flat.value().steps) {
            ...
            if (!family.ok()) {
                return fail(compose, "staged recipe: step '" + step_id +
    ...
    emit_compose(const ComposeIr& compose, std::span<const TheoryIr> theories,
                 std::span<const ComposeIr> composes = {},
                 FusionStatus status = FusionStatus::Fused,
                 const ParamValues& staged_param_values = {}, std::string_view cipher_var = "x",
        ...
        StatusOr<std::string> cpu =
            DslEmitCpu::emit_theory_header(fused.value().theory(), cipher_var);
        if (!cpu.ok()) {
            return cpu.status();
        }
    ...
    [[nodiscard]] static StatusOr<BenchReport>
    bench_cpu(const ComposeIr& compose, std::span<const TheoryIr> theories,
              std::span<const ComposeIr> composes = {}, const ParamValues& param_values = {},
              std::size_t stream_len = 4096, std::size_t reps = 32,
              std::string_view cipher_var = "x") {
        ...
        // Warmup (not timed).
        {
            Status st = DslIrApplicator::apply_into(fused.value().theory().decrypt_step(),
                                                    cipher_var, fused_env, input, out_a);
            if (!st.ok()) {
                return st;
            }
            st = apply_staged_decrypt(flat.value(), cipher_var, param_values, input, out_a, out_b);
            if (!st.ok()) {
                return st;
            }
        }
        ...
        const double fused_sec = std::chrono::duration<double>(t_fused1 - t_fused0).count();

        const auto t_staged0 = std::chrono::steady_clock::now();
        for (std::size_t r = 0; r < reps; ++r) {
            Status st =
                apply_staged_decrypt(flat.value(), cipher_var, param_values, input, out_a, out_b);
            if (!st.ok()) {
                return st;
            }
        }
        const auto t_staged1 = std::chrono::steady_clock::now();
        const double staged_sec = std::chrono::duration<double>(t_staged1 - t_staged0).count();
        ...
        detail << "cpu_bench fused_eps=" << fused_eps << " staged_eps=" << staged_eps
               ...
               << " reps=" << reps;
        return BenchReport{fused_eps, staged_eps, status, stream_len, reps, detail.str()};
    }

    /// Bench then emit with selected `FusionStatus` (still emits both path texts).
    [[nodiscard]] static StatusOr<EmitBundle>
    emit_compose_auto(const ComposeIr& compose, std::span<const TheoryIr> theories,
                      std::span<const ComposeIr> composes = {},
                      const ParamValues& staged_param_values = {}, std::size_t stream_len = 4096,
                      std::size_t reps = 32, std::string_view cipher_var = "x") {
        StatusOr<BenchReport> bench = bench_cpu(compose, theories, composes, staged_param_values,
                                                stream_len, reps, cipher_var);
        if (!bench.ok()) {
            return bench.status();
        }
        return emit_compose(compose, theories, composes, bench.value().status(),
                            staged_param_values, cipher_var, bench.value());
    }
...
private:
    ...
    [[nodiscard]] static TransformDirection recipe_dir_for(const Flattened& flat,
    ...
    [[nodiscard]] static Z29Expr::Env make_compose_env(const ComposeIr& compose,
    ...
    [[nodiscard]] static Z29Expr::Env
    make_stage_env(const TheoryIr& theory, const std::string& step_id,
    ...
    apply_staged_decrypt(const Flattened& flat, std::string_view cipher_var,
                         const ParamValues& values, std::span<const Index29> input,
                         std::span<Index29> buf_a, std::span<Index29> buf_b) {
    ...
    [[nodiscard]] static const TheoryIr*
    find_theory(const std::unordered_map<std::string, const TheoryIr*>& map,
    ...
    bound_compose_param(const std::string& step_id, const std::string& theory_param,
                        const std::vector<ComposeIr::StepParamBinding>& bindings) {
    ...
    stage_params_json(const TheoryIr& theory, const std::string& step_id,
                      const std::vector<ComposeIr::StepParamBinding>& bindings,
                      const ParamValues& values) {
    ...
    [[nodiscard]] static StatusOr<Flattened> flatten_only(const ComposeIr& compose,
                                                          std::span<const TheoryIr> theories,
                                                          std::span<const ComposeIr> composes) {
        Status st = compose.validate();
        if (!st.ok()) {
        ...
        st = flatten_steps(compose, compose_by_name, out.steps, out.bindings, out.recipe_dirs,
                           out.nested,
                           /*depth=*/0);
        if (!st.ok()) {
            return st;
    ...
    [[nodiscard]] static Status
    flatten_steps(const ComposeIr& compose,
                  ...
                  bool& nested_flattened, std::size_t depth) {
        if (depth > max_depth) {
            return fail(compose, "compose nesting exceeds max_depth " + std::to_string(max_depth));
        }
        for (const std::string& step_id : compose.steps()) {
            ...
            if (it != compose_by_name.end()) {
                ...
                st = flatten_steps(inner, compose_by_name, out_steps, out_bindings, out_dirs,
                                   nested_flattened, depth + 1);
                if (!st.ok()) {
                    return st;
                }
                continue;
    ...
    inline_stage(const TheoryIr& theory, const std::string& step_id,
                 const std::vector<ComposeIr::StepParamBinding>& bindings,
                 const Z29Expr::Ptr& cipher_expr, std::string_view cipher_var, bool use_encrypt) {
        const Z29Expr::Ptr& step = use_encrypt ? theory.encrypt_step() : theory.decrypt_step();
        if (!step) {
            return DslDiag::make(DslRuleId::E032_primitive_body,
    ...
    [[nodiscard]] static StatusOr<std::string>
    emit_staged_cpu_header(const ComposeIr& compose, const Flattened& flat,
    ...
    [[nodiscard]] static StatusOr<std::string>
    emit_staged_cuda_header(const ComposeIr& compose, const Flattened& flat,
                            const ParamValues& param_values) {
        (void)param_values;
        for (const std::string& step_id : flat.steps) {
            StatusOr<std::string> fam = cuda_family_enum(step_id);
            if (!fam.ok()) {
                return fail(compose, "staged ComposeDriver emit: step '" + step_id +
        ...
        for (std::size_t i = 0; i < flat.steps.size(); ++i) {
            ...
            if (step_id == "caesar") {
                ...
                out << "        recipe.stages[" << i << "].totient.prime_start_index = " << idx
                    << ";\n";
            }
            (void)th;
        }
